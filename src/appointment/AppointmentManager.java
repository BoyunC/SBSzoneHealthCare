...
public class AppointmentManager {
	public static void updateAppointment() {
		...
		System.out.println("[system] 예약 변경 완료");
	...
	public static void cancelAppointment() {
		...
		List<Appointment> tempList = tempPatient.getAppointmentList();
		showAppointmentList(tempList);
		System.out.print("[system] 취소할 예약 선택 : ");
		int selectAppoint = Integer.parseInt(DataInput.sc.nextLine()) - 1;
		...
		System.out.println("[system] 예약 취소 완료");
	}

	public static void showAppointmentList(List<Appointment> list) {
		int i = 1;
		for (Appointment a : list) {
			System.out.println("[" + i++ + "]" + a.getDate().get(Calendar.MONTH) + "월 " + a.getDate().get(Calendar.DATE)
					+ "일 " + a.getTime());
		}
	}
	...
	public static void showTodayAppointList() {
		...
		for (Appointment a : Calendars.days.get(Calendar.getInstance().get(Calendar.DATE) - 1).getAppointments()) {
			if (a.getPatient() != null) {
				System.out.println("[" + i++ + "]" + a.getTime() + " " + a.getPatient().getName() + " 님");
			}
		}
	}
	...
	public static List<Appointment> getTodayAppointList() {
		...
		int i = 1;
		// day에서 몇번째인지
		for (Day day : Calendars.days) {
			if (day.getDate() == Calendar.DATE) {

				for (Appointment a : Calendars.days.get(Calendar.getInstance().get(Calendar.DATE) - 1)
						.getAppointments()) {
					if (a.getPatient() != null) {
						temp.add(a);
					}
...
//		for(Appointment a : Calendars.days.get(Calendar.getInstance().get(Calendar.DATE)).getAppointments()) {
//			if(a.getPatient() != null) {
//				temp.add(a);
//			}
//		}
